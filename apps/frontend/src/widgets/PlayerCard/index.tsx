...
export const PlayerCard = ({
  ...
  return (
    <div
      ...
      style={showDivider ? { borderBottom: `1px solid ${PLAYER_CARD_COLORS.divider}` } : undefined}
    >
      ...
      {isExtended ? (
        <div className="flex min-w-0 flex-1 flex-col justify-center gap-2">
          <div className="flex min-w-0 items-center gap-2">
            <Text as="p" size="base" weight="normal" className="truncate font-manrope leading-22" style={{ color: PLAYER_CARD_COLORS.text }}>
              {nickname}
            </Text>
            {role ? (
              <span className="flex items-center justify-center rounded px-10 py-2">
                <Text as="span" size="sm" weight="medium" className="font-manrope leading-16 tracking-[0.28px]" style={{ color: PLAYER_CARD_COLORS.role }}>
                  {role}
                </Text>
              </span>
            ) : null}
          ...
          {(caption || formattedHash) ? (
            <div className="flex min-w-0 items-center gap-2">
              ...
              {formattedHash ? (
                <span className="flex items-center justify-center rounded px-10 py-2">
                  <Text as="span" size="sm" weight="medium" className="font-manrope leading-16 tracking-[0.28px]" style={{ color: PLAYER_CARD_COLORS.hash }}>
                    {formattedHash}
                  </Text>
                </span>
              ) : null}
        ...
        <Text as="p" size="base" weight="normal" className="font-manrope leading-22" style={{ color: PLAYER_CARD_COLORS.text }}>
          {nickname}
        </Text>
      )}
    </div>
