...
function Flag({ src, size }: { src: string; size: 'large' | 'small' }) {
  if (size === 'small') {
    return (
...
function PlayersBadge({ playersAmount }: { playersAmount: number | string }) {
  return (
    <div className="flex shrink-0 items-center gap-8 rounded-md border border-fraction-team-players-border bg-fraction-team-players-bg px-8 py-4">
      <span className="flex h-24 w-24 items-center justify-center overflow-hidden">
        <Image
          src="/general/fraction_people.svg"
          alt="Игроки"
          width={20}
          height={13}
          className="h-13 w-20"
      ...
      <Text
        ...
        size="base"
        weight="semibold"
        className="font-manrope leading-22 text-fraction-card-text-primary"
      >
  ...
  if (variant === 'small') {
    return (
      <div className={`flex w-215 items-center gap-8 ${className}`}>
        ...
        <div
          className={`flex min-w-0 flex-1 flex-col justify-center gap-2 ${smallSide === 'right' ? 'items-end' : 'items-start'}`}
        >
          <Text
            as="p"
            size="base"
            weight="medium"
            className="font-manrope leading-22 whitespace-nowrap text-fraction-card-text-primary"
          >
            {fractionName}
          </Text>
          <Text
            ...
            size="sm"
            weight="medium"
            className="font-manrope leading-16 tracking-[0.28px] whitespace-nowrap text-disabled-light"
          >
            {doctrine}
          </Text>
        </div>
    ...
    <div className={`flex w-588 items-center gap-16 ${className}`}>
      <div className="flex min-w-0 flex-1 items-center gap-16">
        ...
        <Text
          as="p"
          weight="medium"
          className="font-manrope text-[28px] leading-32 text-fraction-card-text-primary"
        >
          {fractionName}
        </Text>
      </div>
