...
export function Button({
  ...
  const resolvedAppearance = isAppearanceSupported(variant, appearance)
    ? appearance
    : DEFAULT_APPEARANCE_MAP[variant];
  const variantClass
    = VARIANT_CLASS_MAP[variant][
      resolvedAppearance as keyof (typeof VARIANT_CLASS_MAP)[typeof variant]
    ];
  ...
  return (
    <button
      ...
      {...restProps}
    >
      {showLeftIcon ? leftIcon || <ArrowIcon direction="left" className={sizeClasses.icon} /> : null}
      {children ? <span className={cn('btn-kit-label', sizeClasses.text)}>{children}</span> : null}
      {showRightIcon ? rightIcon || <ArrowIcon direction="right" className={sizeClasses.icon} /> : null}
    </button>
  );
}
