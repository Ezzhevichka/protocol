...
export const banRoute = async (app: FastifyInstance) => {
	app.post(ApiRoutes.PUNISHMENTS_BAN, async (request: FastifyRequest<{ Body: PunishmentRequest, Headers: { 'content-type': string } }>, reply) => {
		...
		const success = await banPlayer({ victimId, authorId, reason, description, until: parsedUntil, serverId, punishmentType });
