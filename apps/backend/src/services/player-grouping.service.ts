...
export function groupPlayersByTeams(input: {
  players: any[];
  squads: any[];
}) {
  ...
  for (const player of input.players) {
    ...
    const squadId =
      player.raw?.squadID == null ? null : String(player.raw.squadID);
    ...
    let group = result[teamId].squads.find(
      (item) => item.squad.squadId === squadId
    );

    if (!group) {
      group = {
        squad:
          squadMap.get(`${teamId}:${squadId}`) ?? {
            squadId,
            teamId,
            name: `Squad ${squadId}`,
          },
      ...
      squads: team.squads
        .map((squadGroup) => ({
          ...
          players: squadGroup.players.sort(
            (a, b) =>
              Number(Boolean(b.raw?.isLeader)) -
                Number(Boolean(a.raw?.isLeader)) ||
              a.name.localeCompare(b.name)
          ),
        }))
        .sort(
          (a, b) =>
            Number(isCommandSquad(b.squad.name)) -
              Number(isCommandSquad(a.squad.name)) ||
            numericSort(a.squad.squadId, b.squad.squadId)
        ),
      unassigned: team.unassigned.sort((a, b) =>
        a.name.localeCompare(b.name)
      ),
    }));
}