...
async function enrichBans(bans: Ban[]) {
  const steamIds = bans.filter((ban) => ban.targetType === "STEAM_ID").map((ban) => ban.normalizedTargetValue);
  const eosIds = bans.filter((ban) => ban.targetType === "EOS_ID").map((ban) => ban.normalizedTargetValue);
...
export async function getBanStatus(input: { steamId?: string; eosId?: string; nickname?: string }) {
  ...
  if (input.steamId) {
    or.push({ targetType: BanTargetType.STEAM_ID, normalizedTargetValue: normalizeSteamId(input.steamId) });
  }

  if (input.eosId) {
    or.push({ targetType: BanTargetType.EOS_ID, normalizedTargetValue: normalizeEosId(input.eosId) });
  }

  const activeBan = or.length
    ? await prisma.ban.findFirst({ where: { ...activeBanWhere(), OR: or }, orderBy: { createdAt: "desc" } })
    : null;
...
export async function createBan(input: CreateBanInput) {
  ...
  const kickResults = await Promise.allSettled(
    squadServers.map(async (server) => {
      ...
      const matched = players.filter((player) => createdBans.some((ban) => banMatchesPlayer(ban, player)));

      for (const player of matched) {
        await kickPlayerOnBot(server, player.steamId, input.reason);
      }

      return { serverId: server.id, matched: matched.length, kickedSteamIds: matched.map((player) => player.steamId) };
    })
  );
  ...
  return {
    ...
    kickResults: kickResults.map((result, index) => {
      ...
      return { serverId: server.id, matched: 0, kickedSteamIds: [], error: result.reason instanceof Error ? result.reason.message : String(result.reason) };
    }),
  };
}
