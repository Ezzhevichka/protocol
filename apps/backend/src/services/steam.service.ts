...
export async function refreshSteamProfile(steamId: string) {
  ...
  const [summaryResponse, gamesResponse] = await Promise.all([
    ...
    fetch(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${key}&steamid=${steamId}&include_appinfo=true&include_played_free_games=true`),
  ]);

  const summaryJson = await summaryResponse.json().catch(() => null) as any;
  const gamesJson = await gamesResponse.json().catch(() => null) as any;
