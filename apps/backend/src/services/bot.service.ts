...
export async function listPlayersFromBot(
    server: SquadServerConfig
): Promise<{ players: any[]; squads: any[]; server: any }> {
    try {
...
export async function kickPlayerOnBot(
    server: SquadServerConfig,
    steamId: string,
    reason: string
): Promise<unknown> {
...
export async function warnPlayerOnBot(server: SquadServerConfig, steamId: string, message: string) {
    const response = await axios.post(
        `${server.botUrl}/warn`,
        {
            steamId,
