...
export function tailLocalLogFile(filePath: string, onLine: LineHandler) {
  ...
  async function readNewChunk() {
    ...
    const stream = fs.createReadStream(filePath, { start: position, end: stat.size, encoding: "utf8" });
    position = stat.size;
  ...
  watcher.on("change", () => readNewChunk().catch((error) => console.error("LOCAL_LOG_TAIL_FAILED", error)));
  return watcher;
}
